...
def main():
    ...
    for images_name in os.listdir(SAMPLE_DIR):
        ...
        with torch.no_grad():
            ...
            images['lv1'] = transforms.ToTensor()(crop_image(Image.open(SAMPLE_DIR + '/' + images_name).convert('RGB')))
            ...
            # print(images['lv2_1'].shape,residual[s]['lv3_top'].shape)
            ...
            stop = time.time()
            deblur_image = torch.clamp(deblurred_image, -0.5, 0.5)
            test_time += stop - start
            print('[%d/%d]'%(iteration,len(os.listdir(SAMPLE_DIR))) ,'RunTime:%.4f' % (stop - start), '  Average Runtime:%.4f' % (test_time / (iteration + 1)))
            ...
            uiqm += uiqm_
            iteration += 1
    print('UICM:%.3f' % (uicm / len(os.listdir(SAMPLE_DIR))), 'UISM:%.4f' % (uism / len(os.listdir(SAMPLE_DIR))),
          'UICONM:%.3f' % (uiconm / len(os.listdir(SAMPLE_DIR))), 'UIQM:%.3f' % (uiqm / len(os.listdir(SAMPLE_DIR))))
