...
parser.add_argument("-c", "--cropsize", type=int, default=112)
...
def main():
    ...
    for epoch in range(args.start_epoch, EPOCHS):
        ...
        for iteration, inputs in enumerate(train_dataloader):
            ...
            loss = mse(255 * (residual['s3']['lv1']), 255 * (images['gt']))
            temp = loss
            loss += mse(255 * (residual['s2']['lv1']), 255 * (images['gt'])) + mse(255 * (residual['s1']['lv1']),
                                                                                   255 * (images['gt']))
            loss += 100 * hsv(residual['s3']['lv1']).item()
            for s in ['s1', 's2', 's3']:
                for lv in ['lv1', 'lv2', 'lv3']:
            ...
            if (iteration + 1) % 10 == 0:
                print(METHOD + "   epoch:", epoch, "iteration:", iteration + 1, "loss:", temp.item())
        ...
        if (epoch) % 100 == 0:
            ...
            for s in ['s1', 's2', 's3']:
                for lv in ['lv1', 'lv2', 'lv3']:
                    torch.save(encoder[s][lv].state_dict(), str(
                        './checkpoints/' + METHOD + '/epoch' + str(epoch) + "/encoder_" + s + "_" + lv + ".pkl"))
                    torch.save(decoder[s][lv].state_dict(), str(
                        './checkpoints/' + METHOD + '/epoch' + str(epoch) + "/decoder_" + s + "_" + lv + ".pkl"))
            ...
            for iteration, inputs in enumerate(test_dataloader):
                with torch.no_grad():
                    test = Variable(inputs['sharp_image'] - 0.5).cuda(GPU)
                    images['lv1'] = Variable(inputs['blur_image'] - 0.5).cuda(GPU)
                    H = images['lv1'].size(2)
                    W = images['lv1'].size(3)
                    images['lv2_1'] = images['lv1'][:, :, 0:int(H / 2), :]
                    ...
                    feature[s]['lv2_1'] = encoder[s]['lv2'](images['lv2_1'] + residual[s]['lv3_top']) + feature[s][
                        'lv3_top']
                    feature[s]['lv2_2'] = encoder[s]['lv2'](images['lv2_2'] + residual[s]['lv3_bot']) + feature[s][
                        'lv3_bot']
                    feature[s]['lv2'] = torch.cat((feature[s]['lv2_1'], feature[s]['lv2_2']), 2)
                    residual[s]['lv2'] = decoder[s]['lv2'](feature[s]['lv2'])
                    ...
                    feature[s]['lv3_3'] = encoder[s]['lv3'](residual[ps]['lv1'][:, :, int(H / 2):H, 0:int(W / 2)])
                    feature[s]['lv3_4'] = encoder[s]['lv3'](residual[ps]['lv1'][:, :, int(H / 2):H, int(W / 2):W])
                    feature[s]['lv3_top'] = torch.cat((feature[s]['lv3_1'], feature[s]['lv3_2']), 3) + feature[ps][
                        'lv3_top']
                    feature[s]['lv3_bot'] = torch.cat((feature[s]['lv3_3'], feature[s]['lv3_4']), 3) + feature[ps][
                        'lv3_bot']
                    residual[s]['lv3_top'] = decoder[s]['lv3'](feature[s]['lv3_top'])
                    residual[s]['lv3_bot'] = decoder[s]['lv3'](feature[s]['lv3_bot'])
                    ...
                    feature[s]['lv1'] = encoder[s]['lv1'](residual[ps]['lv1'] + residual[s]['lv2']) + feature[s][
                        'lv2'] + feature[ps]['lv1']
                    residual[s]['lv1'] = decoder[s]['lv1'](feature[s]['lv1'])
                    ...
                    te_loss += mse(255 * deblurred_image, 255 * test).item()
                    psnr_test += batch_PSNR(deblurred_image, test, 1.)
                    ssim_test += batch_SSIM(deblurred_image, test, 1.)
        ...
        for s in ['s1', 's2', 's3']:
            for lv in ['lv1', 'lv2', 'lv3']:
                torch.save(encoder[s][lv].state_dict(),
                           str('./checkpoints/' + METHOD + "/encoder_" + s + "_" + lv + ".pkl"))
                torch.save(decoder[s][lv].state_dict(),
                           str('./checkpoints/' + METHOD + "/decoder_" + s + "_" + lv + ".pkl"))
