...
class TrainDataset(Dataset):
    def __init__(self, blur_image_files, sharp_image_files, root_dir, crop=False, crop_size=256, multi_scale=False, rotation=False, color_augment=False, transform=None, resize = False, d=32):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        #print(image_name)
        try:
            blur_image = Image.open(os.path.join(image_name[0], image_name[1], image_name[2])).convert('RGB')
        except:
            print(image_name)
        sharp_image = Image.open(os.path.join(image_name[0],'reference890',image_name[2])).convert('RGB')
        if self.resize:
            blur_image = crop_image(blur_image,self.d)
            sharp_image = crop_image(sharp_image,self.d)
        if self.rotation:
